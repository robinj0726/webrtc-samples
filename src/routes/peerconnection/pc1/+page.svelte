...
<div id="container">
	...
	<div class="box">
		<button id="startButton" on:click={start} disabled={!startButtonEnabled}>Start</button>
		<button id="callButton" on:click={call} disabled={status === CallStatus.STARTED}>Call</button>
		<button id="hangupButton" on:click={hangUp} disabled={status === CallStatus.ENDED}
			>Hang Up</button
		>
	</div>
